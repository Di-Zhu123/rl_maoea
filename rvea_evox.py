...
class RVEA(Algorithm):
    ...
    def step(self):
        ...
        crossovered = aroaop(pop, t=self.gen, maxEvals=self.max_gen, action=self.action, ub=self.ub, lb=self.lb, mode='evox')
        ...
        # randomly select the remaining individuals to keep the population size, also help the aroa offspring generation 
        # if len(survivors) < n_survive:
        #     survivors.extend(np.random.choice(np.where(~pop.get("opt"))[0], n_survive - len(survivors)))
        survivor, survivor_fit = replace_nan_rows(survivor, survivor_fit, merge_pop, merge_fit)
