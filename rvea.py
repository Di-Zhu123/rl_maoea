...
from aroa_algo import AROAAlgorithm
...
class RVEA(AROAAlgorithm):
    ...
    def _set_optimum(self, **kwargs):
        if not has_feasible(self.pop):
            ...
            self.opt = self.pop
    # added in 2.28
    def load_action(self, action, t, maxEvals):
        self.algo_load_action(action, t, maxEvals)
        # self.action = action
...
# Compute pairwise distances efficiently with ANN
    nbrs = NearestNeighbors(n_neighbors=2, algorithm='ball_tree').fit(V)
    distances, indices = nbrs.kneighbors(V)
    gamma = np.arccos(1 - distances[:, 1])  # Convert distance to cosine similarity
    gamma = np.maximum(gamma, 1e-64)
    return gamma
...
class APDSurvival(Survival):
    ...
    def _do(self, problem, pop, n_survive, algorithm=None, n_gen=None, n_max_gen=None, **kwargs):
        ...
        acute_angle = np.arccos(np.clip(F_prime @ self.V.T))
        ...
        # todo: randomly select the remaining individuals to keep the population size, also help the aroa offspring generation 
        if len(survivors) < n_survive:
            survivors.extend(np.random.choice(np.where(~pop.get("opt"))[0], n_survive - len(survivors)))
